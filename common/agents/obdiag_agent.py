...
obdiag 是一款适用于 OceanBase 的黑屏诊断工具，现有功能包含了对于 OceanBase 日志、SQL Audit 以及 OceanBase 进程堆栈等信息进行的扫描、收集，可以在 OceanBase 集群不同的部署模式下（OCP，OBD 或用户根据文档手工部署）实现一键执行，完成诊断信息的获取。
...
----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
obdiag gather run --scene=observer.backup                                                                                           [backup problem]                              [数据备份问题]
obdiag gather run --scene=observer.backup_clean                                                                                     [backup clean]                                [备份清理问题]
obdiag gather run --scene=observer.base                                                                                             [cluster base info]                           [集群基础信息]
obdiag gather run --scene=observer.clog_disk_full                                                                                   [clog disk full]                              [clog盘满]
obdiag gather run --scene=observer.cluster_down                                                                                     [cluster down]                                [集群无法连接]
obdiag gather run --scene=observer.compaction                                                                                       [compaction]                                  [合并问题]
obdiag gather run --scene=observer.cpu_high                                                                                         [High CPU]                                    [CPU高]
obdiag gather run --scene=observer.delay_of_primary_and_backup                                                                      [delay of primary and backup]                 [主备库延迟]
obdiag gather run --scene=observer.io                                                                                               [io problem]                                  [io问题]
obdiag gather run --scene=observer.log_archive                                                                                      [log archive]                                 [日志归档问题]
obdiag gather run --scene=observer.long_transaction                                                                                 [long transaction]                            [长事务]
obdiag gather run --scene=observer.memory                                                                                           [memory problem]                              [内存问题]
obdiag gather run --scene=observer.perf_sql --env "{{db_connect='-h127.0.0.1 -P2881 -utest@test -p****** -Dtest', trace_id='Yxx'}}"   [SQL performance problem]                     [SQL性能问题]
obdiag gather run --scene=observer.px_collect_log --env "{{trace_id='Yxx', estimated_time='2024-04-19 14:46:17'}}"                    [Collect error source node logs for SQL PX]   [SQL PX 收集报错源节点日志]
obdiag gather run --scene=observer.recovery                                                                                         [recovery]                                    [数据恢复问题]
obdiag gather run --scene=observer.restart                                                                                          [restart]                                     [observer无故重启]
obdiag gather run --scene=observer.rootservice_switch                                                                               [rootservice switch]                          [有主改选或者无主选举的切主]
obdiag gather run --scene=observer.sql_err --env "{{db_connect='-h127.0.0.1 -P2881 -utest@test -p****** -Dtest', trace_id='Yxx'}}"    [SQL execution error]                         [SQL 执行出错]
obdiag gather run --scene=observer.suspend_transaction                                                                              [suspend transaction]                         [悬挂事务]
obdiag gather run --scene=observer.unit_data_imbalance                                                                              [unit data imbalance]                         [unit迁移/缩小 副本不均衡问题]
obdiag gather run --scene=observer.unknown                                                                                          [unknown problem]                             [未能明确问题的场景]
----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
日志采集场景: 长事务
日志采集命令: obdiag gather run --scene=observer.long_transaction
...
obdiag_agent = AgentBase(prompt=prompt, name=__name__)
