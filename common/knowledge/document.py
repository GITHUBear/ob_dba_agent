...
class DocumentBase:
    def __init__(self, url: str, path_name_handler: Callable[[str], str] = parse_title):
        self.url = url
...
class Document:
    ...
    def doc_url_enhance(self):
        if self.doc_base.path_name_handler is not None:
            return " - ".join(
                [self.doc_base.path_name_handler(path) for path in self.url_path_list]
            )
        else:
...
class Chunk:
    ...
    def get_enhance_text(self):
        return (
            self.doc.name
            + " - "
            + self.get_enhanced_title_for_embed()
            + ": "
            + self.text
        )
